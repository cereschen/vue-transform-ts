...
export function start(rootPath: string, outPath?:string) {
  const project = new Project({ useInMemoryFileSystem: true, compilerOptions: { allowJs: true } })
  if(!outPath)  outPath = rootPath + 'Ts'
  fileDisplay(rootPath, (file) => {
    if(!outPath) return;
    let isVue = file.match(/\.vue$/)
    ...
    if (isVue) {
      ...
      if (!match) return
      sf = project.createSourceFile(file + '.script.ts', match.groups?.script)
      result = transformScript(sf, rootPath, true)
    } else if (file.match(/(?:mixin)[^]*\.(js|ts)$/i)) {
      needTransform = true
    ...
    if (isVue) {
      if (!match) return
      let filePath = path.relative(rootPath, sf.getFilePath())
      let fileName = sf.getBaseName().replace(/\.script\.ts$/, '')
      fs.ensureFileSync(path.join(outPath, filePath, '../', fileName))
